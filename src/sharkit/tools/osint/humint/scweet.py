...
class ScweetTool(Tool):
    metadata = ToolMetadata(
        name="scweet",
        description="Scweet - Twitter/X scraping tool for OSINT research",
        category="osint.humint.social",
        author="sharkit",
        version="0.1.0",
        safety="safe",
        color="#1ABC9C",
        install=ToolInstallSpec(git_url=None, pip_args=["Scweet"]),
    )
    ...
    def __init__(self) -> None:
        self._options: dict[str, OptionDefinition] = {
            "query": OptionDefinition(
                name="query",
                description="Search query for Twitter/X",
                required=True,
            ),
    ...
    def execute(self, context: ExecutionContext) -> Result:
        ...
        limit = context.options.get("limit") or "100"
        cmd = ["scweet", "search", f'"{query}"', "--limit", limit]

        install_dir = manager.install_path(name)
        venv_bin = install_dir / "venv" / "bin"
        tool_bin = venv_bin / "scweet"
