...
class MailAccessTool(Tool):
    metadata = ToolMetadata(
        name="mailaccess",
        description="MailAccess - email investigation and harvesting tool",
        category="osint.humint.email",
        author="sharkit",
        version="0.1.0",
        safety="safe",
        color="#F39C12",
        install=ToolInstallSpec(git_url=None, pip_args=["mailaccess"]),
    )
    ...
    def __init__(self) -> None:
        self._options: dict[str, OptionDefinition] = {
            ...
            "mode": OptionDefinition(
                name="mode",
                description="Operation mode: investigate or harvest",
                required=False,
                default="investigate",
                choices=["investigate", "harvest"],
            ),
        }
    ...
    def execute(self, context: ExecutionContext) -> Result:
        ...
        mode = context.options.get("mode") or "investigate"
        if mode == "investigate":
            cmd = ["mailaccess", "investigate", email]
        else:
            domain = email.split("@")[-1] if "@" in email else email
            cmd = ["mailaccess", "harvest-emails", "--domain", domain]

        install_dir = manager.install_path(name)
        venv_bin = install_dir / "venv" / "bin"
        tool_bin = venv_bin / "mailaccess"
