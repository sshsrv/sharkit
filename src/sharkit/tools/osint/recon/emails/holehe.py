...
class HoleheTool(Tool):
    metadata = ToolMetadata(
        name="holehe",
        description="Holehe — check if an email is used on various websites",
        category="recon",
        author="megadose",
        version="0.1.0",
        safety="safe",
        ...
        install=ToolInstallSpec(
            git_url="https://github.com/megadose/holehe.git",
            pip_args=["holehe"],
        ),
    )
    ...
    def execute(self, context: ExecutionContext) -> Result:
        ...
        holehe_bin = install_dir / "venv" / "bin" / "holehe"
        if not holehe_bin.exists():
            return Result(
                success=False,
                error=f"Holehe binary not found: {holehe_bin}",
            )
