...
class SocialScanTool(Tool):
    metadata = ToolMetadata(
        name="socialscan",
        description="Check email/user registration across social media platforms",
        category="osint.humint.social",
        author="sharkit",
        version="0.1.0",
        safety="safe",
        color="#E74C3C",
        install=ToolInstallSpec(git_url=None, pip_args=["socialscan"]),
    )
    ...
    def __init__(self) -> None:
        self._options: dict[str, OptionDefinition] = {
            "query": OptionDefinition(
                name="query",
                description="Email address or username to search",
                required=True,
            ),
            "mode": OptionDefinition(
                name="mode",
                description="Search mode: email or username",
                required=False,
                default="email",
    ...
    def execute(self, context: ExecutionContext) -> Result:
        ...
        mode = context.options.get("mode") or "email"
        if mode == "email":
            cmd = ["socialscan", "--emails", query]
        else:
            cmd = ["socialscan", "--users", query]

        install_dir = manager.install_path(name)
        venv_bin = install_dir / "venv" / "bin"
        tool_bin = venv_bin / "socialscan"
