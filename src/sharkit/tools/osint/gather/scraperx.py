...
class ScraperxTool(Tool):
    metadata = ToolMetadata(
        name="scraperx",
        description="ScraperX - web scraping tool for extracting data from URLs and social media",
        category="osint.humint.web",
        author="sharkit",
        version="0.1.0",
        safety="safe",
        color="#2ECC71",
        install=ToolInstallSpec(git_url=None, pip_args=["scraperx"]),
    )
    ...
    def __init__(self) -> None:
        self._options: dict[str, OptionDefinition] = {
            "query": OptionDefinition(
                name="query",
                description="URL, @username, or search query to scrape",
                required=True,
            ),
        }
