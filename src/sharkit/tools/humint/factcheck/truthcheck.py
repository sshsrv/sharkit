...
class TruthCheckTool(Tool):
    metadata = ToolMetadata(
        name="truthcheck",
        description="TruthCheck — verify claims and URLs using LLM-powered fact-checking",
        category="humint",
        author="sharkit",
        version="0.1.0",
        safety="moderate",
        color="#E74C3C",
        install=ToolInstallSpec(git_url=None, pip_args=["truthcheck"]),
    )
    ...
    def __init__(self) -> None:
        self._options: dict[str, OptionDefinition] = {
            "query": OptionDefinition(
                name="query",
                description="Claim text or URL to verify",
                required=True,
            ),
            "mode": OptionDefinition(
                name="mode",
                description="Verification mode: claim or url",
                required=False,
                default="claim",
                choices=["claim", "url"],
            ),
    ...
    def execute(self, context: ExecutionContext) -> Result:
        ...
        mode = context.options.get("mode") or "claim"
        cmd = ["truthcheck", "verify", query] if mode == "url" else ["truthcheck", "check", query]

        install_dir = manager.install_path(name)
        venv_bin = install_dir / "venv" / "bin"
        tool_bin = venv_bin / "truthcheck"
